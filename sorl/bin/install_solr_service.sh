...
SOLR_DIR="$SOLR_EXTRACT_DIR/$SOLR_DIR"
...
cp "bin/init.d/solr" "/etc/init.d/$SOLR_SERVICE"
...
if [ -f "$SOLR_VAR_DIR/data/solr.xml" ]; then
  ...
  cp "$SOLR_DIR/server/solr/solr.xml" "$SOLR_VAR_DIR/data/solr.xml"
fi
if [ -f "$SOLR_VAR_DIR/log4j.properties" ]; then
  echo -e "\n$SOLR_VAR_DIR/log4j.properties already exists. Skipping install ...\n"
else
  cp "$SOLR_DIR/server/resources/log4j.properties" "$SOLR_VAR_DIR/log4j.properties"
fi
