...
cp "$PROJECT_SOLR_DIR/bin/init.d/solr" "/etc/init.d/$SOLR_SERVICE"
...
if [ -f "$SOLR_VAR_DIR/solr.in.sh" ]; then
  ...
  cp "$PROJECT_SOLR_DIR/bin/solr.in.sh" "/etc/default/$SOLR_SERVICE.in.sh"
...
if [ -f "$SOLR_VAR_DIR/data/solr.xml" ]; then
  ...
  cp "$PROJECT_SOLR_DIR/server/solr/solr.xml" "$SOLR_VAR_DIR/data/solr.xml"
fi
if [ -f "$SOLR_VAR_DIR/log4j.properties" ]; then
  echo -e "\n$SOLR_VAR_DIR/log4j.properties already exists. Skipping install ...\n"
else
  cp "$PROJECT_SOLR_DIR/server/resources/log4j.properties" "$SOLR_VAR_DIR/log4j.properties"
fi
