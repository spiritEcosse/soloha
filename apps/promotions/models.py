...
class LinkedPromotion(models.Model):
    # We use fields foreign key to link to a promotion model
...
class AbstractPromotion(models.Model):
    ...
    """
    _type = 'Promotion'
    keywords = fields.GenericRelation(KeywordPromotion, verbose_name=_('Keywords'))
...
class RawHTML(AbstractPromotion):
    ...
    display_type = models.CharField(
        ...
        help_text=_(
            "This can be used to have different types of HTML blocks (eg different widths)"
        )
    )
...
class Image(AbstractPromotion):
    ...
    image = models.ImageField(
        _('Image'), upload_to=settings.PROMOTION_FOLDER,
        max_length=255)
    date_created = models.DateTimeField(auto_now_add=True)
