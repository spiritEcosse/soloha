...
class BootstrapPagerNode(Node):
    ...
    def render(self, context):
        ...
        if page.has_previous():
            previous_page_url = get_page_url(page.previous_page_number(), context.current_app, url_view_name,
                                             url_extra_args, url_extra_kwargs, url_param_name, url_get_params,
                                             url_anchor)

        next_page_url = None
        if page.has_next():
            next_page_url = get_page_url(page.next_page_number(), context.current_app, url_view_name, url_extra_args,
                                         url_extra_kwargs, url_param_name, url_get_params, url_anchor)
    ...
    def render(self, context):
        ...
        for curpage in page_range:
            if not show_index_range:
                ...
                index_range = "%s-%s" % (
                1 + (curpage - 1) * page.paginator.per_page, curpage * page.paginator.per_page,)

            url = get_page_url(curpage, context.current_app, url_view_name, url_extra_args, url_extra_kwargs,
                               url_param_name, url_get_params, url_anchor)
            page_urls.append((curpage, index_range, url))

        first_page_url = None
        if current_page >= 1:
            first_page_url = get_page_url(1, context.current_app, url_view_name, url_extra_args, url_extra_kwargs,
                                          url_param_name, url_get_params, url_anchor)

        last_page_url = None
        if current_page <= page_count:
            last_page_url = get_page_url(page_count, context.current_app, url_view_name, url_extra_args,
                                         url_extra_kwargs, url_param_name, url_get_params, url_anchor)
