    ...
    def create_feature(self, cursor):
        ...
        for key_product, row_product in enumerate(products_rows):
            ...
            if product is not None:
                print product.slug, product.pk
                cursor.execute('SElect * from `product_related` WHERE product_id={}'.format(row_product.product_id))

                for recommend_product in namedtuplefetchall(cursor):
                    fetch_product = self.get_product(cursor, recommend_product.related_id)

                    if fetch_product is not None:
                        try:
                            ProductRecommendation.objects.create(primary=product, recommendation=fetch_product)
                            print 'related product - {}'.format(fetch_product)
                        except IntegrityError as e:
                            print e
                            print 'product - {}, fetch_product - {} \n\n'.format(product, fetch_product)
