...
class Basket(models.Model):
    ...
    owner = models.ForeignKey(AUTH_USER_MODEL, related_name='baskets', null=True, verbose_name=_("Owner"))
    ...
    status = models.CharField(_("Status"), max_length=128, default=OPEN, choices=STATUS_CHOICES, db_index=True)
    ...
    vouchers = models.ManyToManyField(Voucher, verbose_name=_("Vouchers"), blank=True)

    date_created = models.DateTimeField(_("Date created"), auto_now_add=True, db_index=True)
    date_merged = models.DateTimeField(_("Date merged"), null=True, blank=True, db_index=True)
    date_submitted = models.DateTimeField(_("Date submitted"), null=True, blank=True, db_index=True)
...
class Line(models.Model):
    ...
    product = models.ForeignKey(Product, related_name='basket_lines', verbose_name=_("Product"))

    # We store the stockrecord that should be used to fulfil this line.
    stockrecord = models.ForeignKey(StockRecord, related_name='basket_lines')

    quantity = models.PositiveIntegerField(_('Quantity'), default=1, db_index=True)
    ...
    price_currency = models.CharField(_("Currency"), max_length=12, default=get_default_currency, db_index=True)
    price_excl_tax = models.DecimalField(_('Price excl. Tax'), decimal_places=2, max_digits=12, null=True)
    price_incl_tax = models.DecimalField(_('Price incl. Tax'), decimal_places=2, max_digits=12, null=True)
