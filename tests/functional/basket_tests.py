...
from apps.basket import reports
from test.basket import add_product
from test import factories
from apps.partner import strategy
...
class AnonAddToBasketViewTests(TestCase):
    ...
    def test_cookie_is_created(self):
        self.assertTrue('soloha_open_basket' in self.response.cookies)

    def test_price_is_recorded(self):
        basket_id = self.response.cookies['soloha_open_basket'].value.split(':')[0]
...
class BasketThresholdTest(TestCase):
    ...
    def setUp(self):
        self._old_threshold = settings.soloha_MAX_BASKET_QUANTITY_THRESHOLD
        settings.soloha_MAX_BASKET_QUANTITY_THRESHOLD = 3

    def tearDown(self):
        settings.soloha_MAX_BASKET_QUANTITY_THRESHOLD = self._old_threshold
    ...
    def test_adding_more_than_threshold_raises(self):
        ...
        self.assertTrue('soloha_open_basket' in response.cookies)
        post_params = {'product_id': dummy_product.id,
                       'action': 'add',
                       'quantity': 2}
        response = self.client.post(url, post_params)
