...
from test.factories import PartnerFactory, ShippingAddressFactory
from test.factories import create_order, create_basket
from test.testcases import WebTestCase
from test.factories import SourceTypeFactory
...
class TestOrderListSearch(WebTestCase):
    ...
    TEST_CASES = [
        ...
        (
            # Assumes that the test settings (soloha_ORDER_STATUS_PIPELINE)
...
class TestOrderDetailPage(WebTestCase):
    ...
    def test_allows_line_status_to_be_changed(self):
        ...
        self.assertEqual(line.status, settings.soloha_INITIAL_LINE_STATUS)
    ...
    def test_allows_order_status_to_be_changed(self):
        ...
        self.assertEqual(
            self.order.status, settings.soloha_INITIAL_ORDER_STATUS)
