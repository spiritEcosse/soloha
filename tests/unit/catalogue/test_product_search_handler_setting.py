...
from apps.catalogue.search_handlers import (
...
class TestProductSearchHandlerSetting(TestCase):
    def test_product_search_handler_setting(self):
        """
        Test that the `soloha_PRODUCT_SEARCH_HANDLER` setting, when set,
        ...
        with override_settings(soloha_PRODUCT_SEARCH_HANDLER=handler_override):
