...
import soloha
from soloha.core.loading import (
...
class TestClassLoading(TestCase):
    """
    soloha's class loading utilities
    """

    def test_load_soloha_classes_correctly(self):
        Product, Category = get_classes('catalogue.models', ('Product', 'Category'))
        self.assertEqual('apps.catalogue.models', Product.__module__)
        self.assertEqual('apps.catalogue.models', Category.__module__)

    def test_load_soloha_class_correctly(self):
        Product = get_class('catalogue.models', 'Product')
        self.assertEqual('apps.catalogue.models', Product.__module__)

    def test_load_soloha_class_from_dashboard_subapp(self):
        ReportForm = get_class('dashboard.reports.forms', 'ReportForm')
        self.assertEqual('apps.dashboard.reports.forms', ReportForm.__module__)
    ...
    def test_raise_importerror_if_app_raises_importerror(self):
        """
        This tests that soloha doesn't fall back to using the soloha catalogue
        app if the overriding app throws an ImportError.
        """
        apps = list(settings.INSTALLED_APPS)
        apps[apps.index('apps.catalogue')] = 'tests._site.import_error_app.catalogue'
...
class ClassLoadingWithLocalOverrideTests(TestCase):
    ...
    def setUp(self):
        ...
        self.installed_apps[self.installed_apps.index('apps.shipping')] = 'tests._site.shipping'
    ...
    def test_loading_class_which_is_not_defined_in_local_module(self):
        with override_settings(INSTALLED_APPS=self.installed_apps):
            ...
            self.assertEqual('apps.shipping.methods', FixedPrice.__module__)

    def test_loading_class_from_module_not_defined_in_local_app(self):
        with override_settings(INSTALLED_APPS=self.installed_apps):
            (Repository,) = get_classes('shipping.repository', ('Repository',))
            self.assertEqual('apps.shipping.repository', Repository.__module__)

    def test_loading_classes_defined_in_both_local_and_soloha_modules(self):
        with override_settings(INSTALLED_APPS=self.installed_apps):
    ...
    def test_overriding_view_is_possible_without_overriding_app(self):
        from apps.customer.app import application, CustomerApplication
...
class ClassLoadingWithLocalOverrideWithMultipleSegmentsTests(TestCase):
    ...
    def setUp(self):
        self.installed_apps = list(settings.INSTALLED_APPS)
        self.installed_apps[self.installed_apps.index('apps.shipping')] = 'tests._site.apps.shipping'
...
class TestGetCoreAppsFunction(TestCase):
    """
    soloha.get_core_apps function
    """

    def test_returns_core_apps_when_no_overrides_specified(self):
        apps = soloha.get_core_apps()
        self.assertEqual(soloha.soloha_CORE_APPS, apps)
    ...
    def test_uses_dashboard_override_when_specified(self):
        apps = soloha.get_core_apps(overrides=['apps.dashboard.catalogue'])
        self.assertTrue('apps.dashboard.catalogue' in apps)
        self.assertTrue('apps.dashboard.catalogue' not in apps)
        self.assertTrue('apps.catalogue' in apps)
...
class TestAppLabelsForModels(TestCase):
    ...
    def test_all_soloha_models_have_app_labels(self):
        from django.apps import apps
        models = apps.get_models()
        missing = []
        for model in models:
            # Ignore non-soloha models
            if not 'soloha' in repr(model):
                continue
            # Don't know how to get the actual model's Meta class. But if
            # the parent doesn't have a Meta class, it's doesn't have an
            # base in soloha anyway and is not intended to be overridden
