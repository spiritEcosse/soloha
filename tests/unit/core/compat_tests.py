...
from soloha.core.compat import UnicodeCSVWriter, existing_user_fields
...
class TestPython3Compatibility(TestCase):
    ...
    def test_models_define_python_3_compatible_representation(self):
        ...
        models = [
            model for model in apps.get_models() if 'soloha' in repr(model)]
        invalid_models = []
        for model in models:
            # Use abstract model if it exists
            if 'soloha' in repr(model.__base__):
