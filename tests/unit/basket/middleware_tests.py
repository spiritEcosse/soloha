...
from apps.basket import middleware
...
class TestBasketMiddleware(TestCase):
    ...
    def test_get_cookie_basket_handles_invalid_signatures(self):
        ...
        request_factory.cookies['soloha_open_basket'] = '1:NOTAVALIDHASH'
        request = request_factory.get('/')
        request.cookies_to_delete = []

        cookie_basket = self.middleware.get_cookie_basket("soloha_open_basket", request, None)

        self.assertEqual(None, cookie_basket)
        self.assertIn("soloha_open_basket", request.cookies_to_delete)